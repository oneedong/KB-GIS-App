...
// ─── Navbar ───────────────────────────────────────────────
function Navbar({ active, onHome, onToday, onCategory, onSearch, onBookmarks }) {
    ...
    return (React.createElement("div", { style: { flexShrink: 0, height: 64, background: 'rgba(255,255,255,0.95)', backdropFilter: 'blur(10px)', borderTop: '1px solid #ece9e2', display: 'flex', alignItems: 'center', justifyContent: 'space-around', paddingBottom: 'max(env(safe-area-inset-bottom), 6px)', boxSizing: 'content-box' } },
        ...
        React.createElement("div", { onClick: onToday, style: tab },
            ...
            React.createElement("span", { style: { font: '600 10.5px Pretendard', color: active === 'today' ? on : off } }, "\uC624\uB298")),
...
function App() {
    ...
    const todayMD = (() => { const k = new Date(Date.now() + 9 * 3600 * 1000); const p = (n) => String(n).padStart(2, '0'); return `${p(k.getUTCMonth() + 1)}.${p(k.getUTCDate())}`; })();
    const todayItems = items.filter(i => i.date === todayMD);
    // Filter
    const isGroup = GROUPS.includes(filter);
    const isGP = GP_NAMES.includes(filter);
    ...
    if (filter !== '전체') {
        if (filter === '인사')
            ...
            feedItems = items.filter(i => i.inst === filter);
        else if (isGroup)
    ...
    const navProps = { onHome: () => goTab('home'), onToday: () => goTab('today'), onCategory: () => goTab('category'), onSearch: () => goTab('search'), onBookmarks: () => goTab('bookmarks') };
    return (React.createElement("div", { className: "app-frame", style: { color: '#1c1d1f' } },
        ...
        screen === 'today' && (React.createElement("div", { style: { flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', background: '#fff' } },
            ...
            React.createElement("div", { style: { flex: 1, minHeight: 0, overflowY: 'auto' } }, todayItems.length === 0 ? (React.createElement("div", { style: { padding: '80px 30px', textAlign: 'center' } },
                ...
                React.createElement("div", { style: { font: '500 12px Pretendard', color: '#a6a8ac', marginTop: 6, lineHeight: 1.5 } },
                    ...
                    "\uD648\uC5D0\uC11C \uC804\uCCB4 \uB274\uC2A4\uB97C \uD655\uC778\uD558\uC138\uC694"))) : todayItems.map(item => React.createElement(FeedItem, { key: item.id, item: item, onOpen: () => openItem(item.id), onBookmark: e => toggleBm(item.id, e) }))),
        ...
        screen === 'category' && (React.createElement("div", { style: { flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', background: '#fff' } },
            ...
            React.createElement("div", { style: { flex: 1, minHeight: 0, overflowY: 'auto', padding: 18 } },
                React.createElement("div", { style: { font: '700 11px Pretendard', color: '#a6a8ac', letterSpacing: '.06em', marginBottom: 10 } }, "\uAD6D\uB0B4 LP \u00B7 \uC5C5\uAD8C\uBCC4"),
        ...
        screen === 'detail' && sel && (React.createElement("div", { style: { flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', background: '#fff' } },
            ...
            React.createElement("div", { style: { flex: 1, minHeight: 0, overflowY: 'auto' } },
                React.createElement("div", { style: { padding: '18px 20px 26px' } },
                    ...
                    React.createElement("a", { href: sel.url, target: "_blank", rel: "noopener noreferrer", style: { display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, marginTop: 16, height: 48, background: '#FFCC00', borderRadius: 12, font: '700 14px Pretendard', color: '#1c1d1f', textDecoration: 'none' } }, "\uAE30\uC0AC \uC804\uBB38 \uBCF4\uAE30 \u2197"),
                    React.createElement("div", { style: { font: '500 11px Pretendard', color: '#b6b8bc', textAlign: 'center', marginTop: 8 } }, "\uC704 \uB0B4\uC6A9\uC740 \uB274\uC2A4 \uC694\uC57D\uC785\uB2C8\uB2E4 \u00B7 \uC804\uCCB4 \uAE30\uC0AC\uB294 \uC6D0\uBB38\uC5D0\uC11C \uD655\uC778\uD558\uC138\uC694"),
