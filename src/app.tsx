...
function flagshipKeywords(core, gpName) {
  ...
  const push = (kw) => {
    ...
    if (k.length < 3) return;              // 너무 짧은 토큰 제외
    if (!/[A-Za-z]/.test(k)) return;       // 라틴 문자가 있어야 함
    if (ROMAN_RE.test(k) || /^\d+$/.test(k)) return;
    if (FLAGSHIP_STOP.has(kl)) return;
    // 구성 단어가 모두 일반명사/지명이면(예: "North America", "Capital Partners") 제외 — 오탐 방지
    if (k.split(/\s+/).every(w => FLAGSHIP_STOP.has(w.toLowerCase()))) return;
    if (kl === gp || gpWords.has(kl)) return;   // GP 자기 이름/티커는 플래그십 특정성이 없음
  ...
  if (toks.length >= 2 && (ROMAN_RE.test(last) || /^\d+$/.test(last))) {
    ...
    if (base.indexOf(' ') !== -1 || base === base.toUpperCase()) push(base);
  }
  return kws;
}
...
function parseFlagshipFunds(flagship, gpName) {
  ...
  return String(flagship).split('·').map(s => s.trim()).filter(Boolean).map(seg => {
    ...
    while (toks.length && /[가-힣]/.test(toks[0])) toks.shift();
    while (toks.length && /[가-힣]/.test(toks[toks.length - 1])) toks.pop();
    ...
    <div style={{flex:1, minHeight:0, display:'flex', flexDirection:'column', background:'#fff'}}>
      ...
      <div style={{flex:1, minHeight:0, overflowY:'auto'}}>
        <div style={{padding:'18px 20px 28px', maxWidth:760, margin:'0 auto'}}>
          ...
          {/* 플래그십 펀드 — 개별 펀드로 분해 + 펀드명이 언급된 관련 뉴스 자동 연동 */}
            ...
            return (
              <div style={{marginTop:10, background:'#fffaeb', border:'1px solid #f6ecc8', borderRadius:11, padding:'12px 13px'}}>
                <div style={{display:'flex', alignItems:'center', gap:7, marginBottom:8}}>
                  <div style={{font:'700 10px Pretendard', color:'#9a7d12', letterSpacing:'.04em'}}>플래그십 펀드</div>
                  {allNews.length > 0 && (
                    <span style={{font:'700 9px Pretendard', color:recent ? '#1a7a4a' : '#9a7d12', background:recent ? '#e4f5ea' : '#fff5d6', padding:'2px 7px', borderRadius:5}}>
                      {recent ? `업데이트 · ${latestY.m}.${pad2(latestY.d)}` : `관련 뉴스 ${allNews.length}`}
                    </span>
                  )}
                  <span style={{marginLeft:'auto', font:'500 9px Pretendard', color:'#c2b06a'}}>뉴스 자동 연동</span>
                </div>
                <div style={{display:'flex', flexDirection:'column', gap:9}}>
                  {funds.map((f, i) => (
                    <div key={i} style={{borderTop:i ? '1px solid #f2e6bf' : 'none', paddingTop:i ? 9 : 0}}>
                      <div style={{font:'600 12.5px/1.55 Pretendard', color:'#3d3e42'}}>{f.label}</div>
                      {f.news.length > 0 && (
                        <div style={{marginTop:6, display:'flex', flexDirection:'column', gap:5}}>
                          {f.news.slice(0, 3).map(a => {
                            const isNew = nowMs - itemMs(a) < 30 * 86400000;
                            return (
                              <div key={a.id} onClick={() => onOpenArticle(a.id)} style={{display:'flex', gap:6, alignItems:'flex-start', cursor:'pointer', background:'#fff', border:'1px solid #f1e7c6', borderRadius:8, padding:'7px 9px'}}>
                                <span style={{width:5, height:5, borderRadius:2, marginTop:5, flexShrink:0, background:(ASSET[a.asset] && ASSET[a.asset].color) || '#c4a93a', display:'inline-block'}}></span>
                                <div style={{flex:1, minWidth:0}}>
                                  <div style={{font:'600 11.5px/1.4 Pretendard', color:'#1c1d1f'}}>{a.ko}</div>
                                  <div style={{display:'flex', alignItems:'center', gap:5, marginTop:3}}>
                                    {isNew && <span style={{font:'700 8px Pretendard', color:'#9a7d12', background:'#FFCC00', borderRadius:3, padding:'1px 4px', letterSpacing:'.04em'}}>NEW</span>}
                                    <span style={{font:'500 9.5px Pretendard', color:'#b6a55f'}}>{a.date} · {a.source}</span>
                                  </div>
                                </div>
                              </div>
                            );
                ...
                <div style={{font:'500 9.5px/1.5 Pretendard', color:'#c2b06a', marginTop:9}}>
                  {allNews.length > 0 ? '펀드명이 언급된 기사를 자동 매칭해 표시합니다.' : '플래그십 펀드가 언급된 새 기사가 수집되면 여기에 자동 표시됩니다.'}
                </div>
              </div>
            );
          })()}
