...
const GP_NAMES = ['Blackstone', 'Apollo', 'KKR', 'BlackRock', 'Carlyle', 'Brookfield', 'Ares', 'EQT', 'CVC', 'TPG', 'Bain Capital', 'Partners Group', 'Oaktree', 'Blue Owl', 'Ardian', 'Stonepeak', 'BBAM'];
...
function Navbar({ active, homeNew, isDesktop, onHome, onToday, onCategory, onKoreaLp, onGlobalGp, onSearch, onBookmarks, onBrief }) {
    ...
    return (React.createElement("div", { style: { flexShrink: 0, height: 64, background: 'rgba(255,255,255,0.95)', backdropFilter: 'blur(10px)', borderTop: '1px solid #ece9e2', display: 'flex', alignItems: 'center', justifyContent: 'space-around', paddingBottom: 'max(env(safe-area-inset-bottom), 6px)', boxSizing: 'content-box' } },
        ...
        React.createElement("div", { onClick: onBrief, style: tab },
            React.createElement("span", { style: { fontSize: 15, lineHeight: 1, color: active === 'brief' ? on : off } }, "\u25A4"),
            React.createElement("span", { style: lbl(active === 'brief') }, "\uC2DC\uD669")),
...
// ─── 실시간 시세 (브라우저에서 직접 조회) ─────────────────
// 지수·환율·원자재는 Yahoo 차트 API 가 CORS 를 막아 공개 프록시를 거치고,
// 크립토는 CORS 가 열린 거래소 API 를 그대로 부른다.
...
// 크립토 — 바이낸스(24h 통계) → 실패 시 코인베이스 현물가
async function liveCrypto() {
    try {
        ...
        if (r.ok) {
            ...
            const out = {};
            for (const x of arr) {
                const sym = x.symbol === 'BTCUSDT' ? 'BTC-USD' : 'ETH-USD';
                out[sym] = { last: parseFloat(x.lastPrice), chg: parseFloat(x.priceChange), chgPct: parseFloat(x.priceChangePercent), src: 'Binance' };
            }
...
// ─── 데일리 시황 표 (지수·환율·원자재·크립토 공통) ────────
// 행을 누르면 5년 추이 그래프가 열린다. live 가 있으면 실시간 값으로 덮어쓴다.
function BriefSection({ title, rows, digits, live, onPick, note }) {
    const list = rows || [];
    return (React.createElement("div", { style: { marginTop: 20 } },
        React.createElement("div", { style: { display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 8, flexWrap: 'wrap' } },
            React.createElement("div", { style: { font: '700 11px Pretendard', color: '#a6a8ac', letterSpacing: '.06em' } }, title),
            note && React.createElement("span", { style: { font: '500 9.5px Pretendard', color: '#9a9ca0' } }, note)),
        React.createElement("div", { style: { border: '1px solid #ece9e2', borderRadius: 13, overflow: 'hidden' } }, list.map((x0, i) => {
            const lv = live && live[x0.symbol];
            const x = lv ? { ...x0, ...lv } : x0;
            const up = x.chgPct != null && x.chgPct > 0, down = x.chgPct != null && x.chgPct < 0;
            const col = up ? '#c0392b' : down ? '#1a5fa4' : '#7a7c80'; // 상승 적색·하락 청색
            const dg = x.unit === '$' ? (x.last != null && x.last < 10 ? 3 : 2) : (digits && !/원|엔/.test(x.unit || '') ? digits : 2);
            return (React.createElement("div", { key: x.name + i, onClick: () => onPick && onPick(x0), style: { display: 'flex', alignItems: 'center', gap: 9, padding: '11px 13px', borderTop: i ? '1px solid #f3f1ea' : 'none', cursor: onPick ? 'pointer' : 'default' } },
                React.createElement("span", { style: { font: '600 12.5px Pretendard', color: '#1c1d1f', flex: 1, minWidth: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' } },
                    x.name,
                    lv && React.createElement("span", { style: { marginLeft: 5, width: 5, height: 5, borderRadius: 999, background: '#1a7a4a', display: 'inline-block', verticalAlign: 'middle' } })),
                React.createElement("span", { style: { font: '800 13.5px Pretendard', color: '#1c1d1f', whiteSpace: 'nowrap' } },
                    x.unit === '$' ? '$' : '',
                    x.last == null ? '–' : x.last.toLocaleString('ko-KR', { minimumFractionDigits: dg, maximumFractionDigits: dg })),
                React.createElement("span", { style: { font: '700 11.5px Pretendard', color: col, minWidth: 66, textAlign: 'right', whiteSpace: 'nowrap' } }, x.chgPct == null ? '–' : `${up ? '▲' : down ? '▼' : ''} ${Math.abs(x.chgPct).toFixed(2)}%`),
                onPick && React.createElement("span", { style: { color: '#cfccc4', flexShrink: 0 } }, "\u203A")));
        }))));
}
// ─── 환헤지 비용 · 스왑포인트 ─────────────────────────────
...
function Sidebar({ active, homeNew, go, onRefresh }) {
    ...
    return (React.createElement("div", { style: { width: 236, flexShrink: 0, background: '#1c1d1f', color: '#fff', display: 'flex', flexDirection: 'column', padding: '22px 14px' } },
        React.createElement("div", { onClick: () => { go('home'); onRefresh(); }, style: { display: 'flex', alignItems: 'center', gap: 9, padding: '4px 12px 22px', cursor: 'pointer' } },
            ...
            React.createElement("div", { style: { padding: '18px 20px 28px', maxWidth: 760, margin: '0 auto' } },
                ...
                React.createElement("div", { style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 9, marginTop: 16 } },
                    React.createElement("div", { style: { background: '#f8f7f3', borderRadius: 11, padding: '12px 13px' } },
                        ...
                        aumNews && aumNews.display && (React.createElement("a", { href: aumNews.url && /^https?:\/\//.test(aumNews.url) ? aumNews.url : undefined, target: "_blank", rel: "noopener noreferrer", style: { display: 'block', textDecoration: 'none', font: '600 10px Pretendard', color: '#1a7a4a', marginTop: 6, lineHeight: 1.5 } },
                            "\u25CF \uB274\uC2A4 \uAE30\uC900 ",
                            aumNews.display,
                            " \u00B7 ",
                            aumNews.date,
                ...
                execs && execs.length > 0 && (React.createElement("div", { style: { marginTop: 20 } },
                    React.createElement("div", { style: { display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 9, flexWrap: 'wrap' } },
                        React.createElement("div", { style: { font: '700 11px Pretendard', color: '#a6a8ac', letterSpacing: '.06em' } }, "\uC6B4\uC6A9\uC870\uC9C1 \uC778\uC0AC"),
                    ...
                    React.createElement("div", { style: { border: '1px solid #ece9e2', borderRadius: 13, overflow: 'hidden' } }, execs.map((e, i) => (React.createElement("a", { key: e.key || i, href: e.url && /^https?:\/\//.test(e.url) ? e.url : undefined, target: "_blank", rel: "noopener noreferrer", style: { display: 'block', textDecoration: 'none', color: 'inherit', padding: '12px 13px', borderTop: i ? '1px solid #f3f1ea' : 'none' } },
                        React.createElement("div", { style: { display: 'flex', alignItems: 'center', gap: 7, flexWrap: 'wrap' } },
                            React.createElement("span", { style: { font: '700 9.5px Pretendard', color: '#56585c', background: '#f0eee7', padding: '3px 9px', borderRadius: 6 } }, e.title),
                            React.createElement("span", { style: { font: '700 13px Pretendard', color: '#1c1d1f' } }, e.person),
                            React.createElement("span", { style: { font: '700 9px Pretendard', color: '#1a5fa4', background: '#e6effa', padding: '2px 7px', borderRadius: 5 } }, e.action)),
                        React.createElement("div", { style: { font: '500 10px Pretendard', color: '#b6b8bc', marginTop: 5 } },
                            e.date,
                            " \u00B7 ",
                            e.source,
                            e.url ? ' · 기사 보기 ↗' : ''))))))),
                move && (React.createElement("div", { style: { marginTop: 20 } },
                    React.createElement("div", { style: { font: '700 11px Pretendard', color: '#a6a8ac', letterSpacing: '.06em', marginBottom: 9 } }, "\uC9C0\uBC29\uC774\uC804 \uC774\uC288"),
                    React.createElement("a", { href: move.url && /^https?:\/\//.test(move.url) ? move.url : undefined, target: "_blank", rel: "noopener noreferrer", style: { display: 'block', textDecoration: 'none', color: 'inherit', border: '1px solid #dde6f3', borderRadius: 13, padding: '13px 14px', background: '#f6f9fe' } },
                        React.createElement("div", { style: { display: 'flex', alignItems: 'center', gap: 7, flexWrap: 'wrap' } },
                            React.createElement("span", { style: { font: '700 9.5px Pretendard', color: '#1a5fa4', background: '#e6effa', padding: '3px 9px', borderRadius: 6 } }, move.stage),
                            React.createElement("span", { style: { marginLeft: 'auto', font: '500 9px Pretendard', color: '#9a9ca0' } }, "\uB274\uC2A4 \uC790\uB3D9")),
                        React.createElement("div", { style: { font: '600 12.5px/1.5 Pretendard', color: '#1c1d1f', marginTop: 7 } }, move.title),
                        React.createElement("div", { style: { font: '500 10px Pretendard', color: '#b6b8bc', marginTop: 5 } },
                            move.date,
                            " \u00B7 ",
                            move.source,
    ...
    return (React.createElement("div", { style: { flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', background: '#fff' } },
        ...
        React.createElement("div", { style: { flex: 1, minHeight: 0, overflowY: 'auto' } },
            React.createElement("div", { style: { padding: '18px 20px 28px', maxWidth: 760, margin: '0 auto' } },
                ...
                profile && profile.people && profile.people.length > 0 && (React.createElement("div", { style: { marginTop: 12, border: '1px solid #ece9e2', borderRadius: 13, overflow: 'hidden' } }, profile.people.map((pp, i) => (React.createElement("div", { key: pp.name + i, style: { padding: '12px 14px', borderTop: i ? '1px solid #f3f1ea' : 'none' } },
                    React.createElement("div", { style: { display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' } },
                        React.createElement("span", { style: { font: '700 13.5px Pretendard', color: '#1c1d1f' } }, pp.name),
                        React.createElement("span", { style: { font: '700 9.5px Pretendard', color: '#56585c', background: '#f0eee7', padding: '3px 9px', borderRadius: 6 } }, pp.title)),
                    pp.note && React.createElement("div", { style: { font: '500 11.5px/1.6 Pretendard', color: '#56585c', marginTop: 7 } }, pp.note)))))),
                profile && profile.note && (React.createElement("div", { style: { marginTop: 10, border: '1px solid #dde6f3', background: '#f6f9fe', borderRadius: 11, padding: '11px 13px' } },
                    React.createElement("div", { style: { font: '700 10px Pretendard', color: '#1a5fa4', letterSpacing: '.04em', marginBottom: 5 } }, "\uCEE4\uBC84\uB9AC\uC9C0 \uB178\uD2B8"),
                    React.createElement("div", { style: { font: '600 12px/1.6 Pretendard', color: '#3d3e42' } }, profile.note))),
                    ...
                    articles.length === 0 ? (React.createElement("div", { style: { font: '500 12px/1.6 Pretendard', color: '#b6b8bc', border: '1px dashed #e3e0d8', borderRadius: 13, padding: '14px' } },
                        ...
                        React.createElement("div", { style: { flex: 1, minWidth: 0 } },
                            React.createElement("div", { style: { display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4, flexWrap: 'wrap' } },
                                ...
                                item.cat === '이전' && React.createElement("span", { style: { font: '700 9px Pretendard', color: '#1a5fa4', background: '#e6effa', padding: '1px 6px', borderRadius: 4 } }, "\uC9C0\uBC29\uC774\uC804"),
                                React.createElement("span", { style: { font: '500 10.5px Pretendard', color: '#bcbec2' } },
                                    item.date,
                                    " ",
                                    item.time)),
...
function App() {
    ...
    const [histSeries, setHistSeries] = useState(null); // history.json 의 계열
    const [trendSel, setTrendSel] = useState(null); // 그래프로 열어 둔 지표
    const [liveQuotes, setLiveQuotes] = useState(null); // 실시간 시세 (심볼별)
    ...
    useEffect(() => {
        fetch(MARKET_API + '?t=' + Date.now())
            .then(r => r.json())
            .then(d => { if (d && (Array.isArray(d.kr) || Array.isArray(d.global)))
            setMarket(d); })
            .catch(() => { });
    }, []);
    // Load 시황 아카이브 목록 (일자별).
    ...
    if (filter !== '전체') {
        if (filter === '인사')
            ...
            feedItems = items.filter(i => i.cat === '이전');
        else if (filter === '마켓')
    ...
    // 지표를 누르면 5년 추이를 연다 — history.json 은 이때 한 번만 받는다.
    const openTrend = (row) => {
        if (!row || !row.symbol)
            return;
        setTrendSel(row);
        if (!histSeries) {
            fetch(HISTORY_API + '?t=' + Date.now())
                .then(r => r.json())
                .then(d => { if (d && d.series)
                setHistSeries(d.series); })
                .catch(() => { });
        }
    };
    // 실시간 시세 — 지수·환율·원자재는 프록시 경유, 크립토는 거래소 API 직접.
    const refreshLive = React.useCallback(async () => {
        ...
        try {
            ...
            if (Object.keys(out).length) {
                setLiveQuotes(out);
                const n = new Date();
                setLiveAt(`${String(n.getHours()).padStart(2, '0')}:${String(n.getMinutes()).padStart(2, '0')}`);
            }
        }
        finally {
            setLiveBusy(false);
    ...
    // 시황 탭에 있는 동안 60초마다 실시간 시세를 갱신한다.
    useEffect(() => {
        if (screen !== 'brief')
            return;
        refreshLive();
    ...
    return (React.createElement("div", { className: "app-frame", style: { color: '#1c1d1f', ...(isDesktop ? { flexDirection: 'row', width: '100vw', height: '100dvh', maxWidth: 'none', borderRadius: 0, border: 'none', boxShadow: 'none' } : {}) } },
        ...
        React.createElement("div", { style: paneStyle },
            ...
            screen === 'korlp' && (lpSel ? (React.createElement(LpProfile, { name: lpSel, group: lpSelGroup, profile: lpSelProfile, alloc: lpSelAlloc, cio: lpSelCio, execs: lpSelExecs, aumNews: lpSelAum, move: lpSelMove, returns: lpSelReturns, articles: lpSelArticles, onBack: () => setLpSel(null), onOpenArticle: (id) => openItemFull(id) })) : (React.createElement("div", { style: { flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', background: '#fff' } },
                React.createElement("div", { style: { flexShrink: 0 } },
                    React.createElement("div", { style: { height: 'max(env(safe-area-inset-top), 8px)', flexShrink: 0 } }),
                ...
                React.createElement("div", { style: { flex: 1, minHeight: 0, overflowY: 'auto', padding: 18, width: '100%', maxWidth: 900, margin: '0 auto', boxSizing: 'border-box' } }, lpTab === 'inst' ? (React.createElement("div", { style: { display: 'flex', flexDirection: 'column', gap: 8 } },
                ...
                !allocSelData ? (React.createElement("div", { style: { padding: '80px 30px', textAlign: 'center' } },
                    ...
                    React.createElement("div", { style: { display: 'flex', alignItems: 'baseline', gap: 8, margin: '26px 0 10px' } },
                        React.createElement("div", { style: { font: '700 11px Pretendard', color: '#a6a8ac', letterSpacing: '.06em' } }, "\uB300\uCCB4\uD22C\uC790 \uC6B4\uC6A9\uC870\uC9C1 \uC778\uC0AC"),
                        React.createElement("span", { style: { font: '500 9.5px Pretendard', color: '#1a7a4a', background: '#e4f5ea', padding: '2px 7px', borderRadius: 5 } }, "\u25CF \uBCF8\uBD80\uC7A5\u00B7\uC2E4\uC7A5\u00B7\uD300\uC7A5\uAE09")),
                    insights && insights.execs && insights.execs.length ? (React.createElement("div", { style: { border: '1px solid #ece9e2', borderRadius: 13, overflow: 'hidden' } }, insights.execs.slice(0, 12).map((e, i) => (React.createElement("div", { key: e.key || i, onClick: () => openLp(e.inst), style: { padding: '12px 13px', borderTop: i ? '1px solid #f3f1ea' : 'none', cursor: 'pointer' } },
                        React.createElement("div", { style: { display: 'flex', alignItems: 'center', gap: 7, flexWrap: 'wrap' } },
                            React.createElement("span", { style: { font: '700 12.5px Pretendard', color: '#1c1d1f' } }, e.inst),
                            React.createElement("span", { style: { font: '700 9.5px Pretendard', color: '#56585c', background: '#f0eee7', padding: '2px 8px', borderRadius: 5 } }, e.title),
                            React.createElement("span", { style: { marginLeft: 'auto', color: '#cfccc4' } }, "\u203A")),
                        React.createElement("div", { style: { font: '500 12px/1.5 Pretendard', color: '#3d3e42', marginTop: 5 } },
                            e.person,
                            " ",
                            e.action),
                        React.createElement("div", { style: { font: '500 10px Pretendard', color: '#b6b8bc', marginTop: 5 } },
                            e.date,
                            " \u00B7 ",
                            e.source)))))) : (React.createElement("div", { style: { font: '500 11px/1.6 Pretendard', color: '#b6b8bc', border: '1px dashed #e3e0d8', borderRadius: 13, padding: '14px' } }, "\uCD5C\uADFC \uAE30\uC0AC\uC5D0\uC11C \uD655\uC778\uB41C \uBCF8\uBD80\uC7A5\u00B7\uC2E4\uC7A5\u00B7\uD300\uC7A5\uAE09 \uC778\uC0AC\uAC00 \uC544\uC9C1 \uC5C6\uC2B5\uB2C8\uB2E4. \uAD00\uB828 \uAE30\uC0AC\uAC00 \uC62C\uB77C\uC624\uBA74 \uC790\uB3D9 \uBC18\uC601\uB429\uB2C8\uB2E4.")),
                    React.createElement("div", { style: { display: 'flex', alignItems: 'baseline', gap: 8, margin: '26px 0 10px' } },
                        React.createElement("div", { style: { font: '700 11px Pretendard', color: '#a6a8ac', letterSpacing: '.06em' } }, "\uC9C0\uBC29\uC774\uC804 \uC774\uC288"),
                        React.createElement("span", { style: { font: '500 9.5px Pretendard', color: '#1a5fa4', background: '#e6effa', padding: '2px 7px', borderRadius: 5 } }, "\u25CF \uACF5\uC81C\uD68C\u00B7\uAD6D\uCC45\uC740\uD589\u00B7\uC5F0\uAE30\uAE08")),
                    insights && insights.relocations && insights.relocations.length ? (React.createElement("div", { style: { border: '1px solid #ece9e2', borderRadius: 13, overflow: 'hidden' } }, insights.relocations.slice(0, 12).map((m, i) => (React.createElement("div", { key: m.inst + i, onClick: () => openLp(m.inst), style: { padding: '12px 13px', borderTop: i ? '1px solid #f3f1ea' : 'none', cursor: 'pointer' } },
                        React.createElement("div", { style: { display: 'flex', alignItems: 'center', gap: 7, flexWrap: 'wrap' } },
                            React.createElement("span", { style: { font: '700 12.5px Pretendard', color: '#1c1d1f' } }, m.inst),
                            React.createElement("span", { style: { font: '700 9px Pretendard', color: '#1a5fa4', background: '#e6effa', padding: '2px 7px', borderRadius: 5 } }, m.stage),
                            React.createElement("span", { style: { marginLeft: 'auto', color: '#cfccc4' } }, "\u203A")),
                        React.createElement("div", { style: { font: '500 12px/1.5 Pretendard', color: '#3d3e42', marginTop: 5 } }, m.title),
                        React.createElement("div", { style: { font: '500 10px Pretendard', color: '#b6b8bc', marginTop: 5 } },
                            m.date,
                            " \u00B7 ",
                            m.source)))))) : (React.createElement("div", { style: { font: '500 11px/1.6 Pretendard', color: '#b6b8bc', border: '1px dashed #e3e0d8', borderRadius: 13, padding: '14px' } }, "\uACF5\uC81C\uD68C\u00B7\uAD6D\uCC45\uC740\uD589 \uC9C0\uBC29\uC774\uC804 \uAD00\uB828 \uAE30\uC0AC\uB97C \uC0C1\uC2DC \uC218\uC9D1\uD569\uB2C8\uB2E4. \uC0C8 \uAE30\uC0AC\uAC00 \uC62C\uB77C\uC624\uBA74 \uC790\uB3D9 \uBC18\uC601\uB429\uB2C8\uB2E4.")),
                    React.createElement("div", { style: { display: 'flex', alignItems: 'baseline', gap: 8, margin: '26px 0 10px' } },
                        React.createElement("div", { style: { font: '700 11px Pretendard', color: '#a6a8ac', letterSpacing: '.06em' } }, "\uC6B4\uC6A9\uC790\uC0B0(AUM) \uCD5C\uC2E0\uD654"),
                        React.createElement("span", { style: { font: '500 9.5px Pretendard', color: '#1a7a4a', background: '#e4f5ea', padding: '2px 7px', borderRadius: 5 } }, "\u25CF \uAE30\uC0AC \uC2A4\uD06C\uB9AC\uB2DD")),
                    insights && insights.aums && insights.aums.length ? (React.createElement(React.Fragment, null,
                        React.createElement("div", { style: { border: '1px solid #ece9e2', borderRadius: 13, overflow: 'hidden' } }, insights.aums.slice(0, 14).map((x, i) => (React.createElement("div", { key: x.inst + i, onClick: () => openLp(x.inst), style: { display: 'flex', alignItems: 'center', gap: 10, padding: '12px 13px', borderTop: i ? '1px solid #f3f1ea' : 'none', cursor: 'pointer' } },
                            React.createElement("span", { style: { font: '600 12px Pretendard', color: '#1c1d1f', flex: 1, minWidth: 0 } }, x.inst),
                            React.createElement("span", { style: { font: '800 13.5px Pretendard', color: '#1c1d1f' } }, x.display),
                            x.ref != null && x.jo != null && (React.createElement("span", { style: { font: '600 10px Pretendard', color: x.jo >= x.ref / 10000 ? '#1a7a4a' : '#c0392b' } },
                                x.jo >= x.ref / 10000 ? '▲' : '▼',
                                " \uACF5\uC2DC ",
                                Math.round(x.ref / 10000),
                                "\uC870")),
                            React.createElement("span", { style: { font: '500 9.5px Pretendard', color: '#b6b8bc' } }, x.date))))),
                        React.createElement("div", { style: { font: '500 10.5px/1.6 Pretendard', color: '#b6b8bc', marginTop: 8 } }, "\u203B \uAE30\uC0AC \uBCF8\uBB38\uC5D0\uC11C \u2018\uC6B4\uC6A9\uC790\uC0B0\u00B7\uAE30\uAE08 \uADDC\uBAA8\u00B7\uC801\uB9BD\uAE08\u2019 \uBB38\uAD6C\uC640 \uD568\uAED8 \uC5B8\uAE09\uB41C \uC218\uCE58\uB9CC \uBC18\uC601\uD569\uB2C8\uB2E4. \uACF5\uC2DC \uD655\uC815\uCE58\uC640 \uB2E4\uB97C \uC218 \uC788\uC5B4 \uAE30\uAD00\uBCC4 \uCD5C\uC2E0 \uACF5\uC2DC\uCE58\uC640 \uD568\uAED8 \uD45C\uC2DC\uD569\uB2C8\uB2E4."))) : (React.createElement("div", { style: { font: '500 11px/1.6 Pretendard', color: '#b6b8bc', border: '1px dashed #e3e0d8', borderRadius: 13, padding: '14px' } }, "\uAE30\uC0AC\uC5D0\uC11C \uD655\uC778\uB41C \uC6B4\uC6A9\uC790\uC0B0 \uC218\uCE58\uAC00 \uC544\uC9C1 \uC5C6\uC2B5\uB2C8\uB2E4. \uAD00\uB828 \uAE30\uC0AC\uAC00 \uC62C\uB77C\uC624\uBA74 \uC790\uB3D9 \uBC18\uC601\uB429\uB2C8\uB2E4.")),
                    React.createElement("div", { style: { display: 'flex', alignItems: 'baseline', gap: 8, margin: '26px 0 10px' } },
                        React.createElement("div", { style: { font: '700 11px Pretendard', color: '#a6a8ac', letterSpacing: '.06em' } }, "\uC790\uC0B0\uAD70\uBCC4 \uC218\uC775\uB960"),
                        React.createElement("span", { style: { font: '500 9.5px Pretendard', color: '#1a7a4a', background: '#e4f5ea', padding: '2px 7px', borderRadius: 5 } }, "\u25CF \uCD5C\uADFC \uAE30\uC0AC \uAE30\uC900")),
            ...
            screen === 'brief' && (() => {
                ...
                const lv = isLatest ? liveQuotes : null; // 과거 시황엔 실시간을 덮지 않는다
                const col2 = { display: 'grid', gridTemplateColumns: isDesktop ? '1fr 1fr' : '1fr', gap: isDesktop ? '0 22px' : 0, alignItems: 'start' };
                return (React.createElement("div", { style: { flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', background: '#fff' } },
                    React.createElement("div", { style: { flexShrink: 0 } },
                        React.createElement("div", { style: { height: 'max(env(safe-area-inset-top), 8px)', flexShrink: 0 } }),
                        React.createElement("div", { style: { padding: '2px 20px 12px', borderBottom: '1px solid #efece4' } },
                            React.createElement("div", { style: { maxWidth: 1120, margin: '0 auto' } },
                                React.createElement("div", { style: { display: 'flex', alignItems: 'baseline', gap: 8, flexWrap: 'wrap' } },
                                    ...
                                    isLatest && (React.createElement("span", { onClick: refreshLive, style: { marginLeft: 'auto', font: '600 10px Pretendard', color: liveAt ? '#1a7a4a' : '#7a7c80', background: liveAt ? '#e4f5ea' : '#f2f0ea', padding: '3px 9px', borderRadius: 999, cursor: 'pointer' } }, liveBusy ? '실시간 조회 중…' : liveAt ? `● 실시간 ${liveAt} · 새로고침` : '실시간 조회 ⟳'))),
                                React.createElement("div", { style: { font: '500 11.5px Pretendard', color: '#9a9ca0', marginTop: 3 } }, b ? `${b.asOf} 기준` : '브리핑을 불러오는 중입니다'),
                                briefIndex && briefIndex.length > 0 && (React.createElement("div", { style: { display: 'flex', gap: 6, overflowX: 'auto', marginTop: 10, paddingBottom: 2 } }, briefIndex.slice(0, 40).map(x => {
                    ...
                    React.createElement("div", { style: { flex: 1, minHeight: 0, overflowY: 'auto' } },
                        React.createElement("div", { style: { padding: isDesktop ? '18px 26px 34px' : '14px 18px 28px', maxWidth: 1120, margin: '0 auto' } }, !b ? (React.createElement("div", { style: { padding: '70px 20px', textAlign: 'center' } },
                            ...
                            React.createElement("div", { style: { marginTop: 14, border: '1px solid #f6ecc8', background: '#fffaeb', borderRadius: 13, padding: '13px 15px' } },
                                ...
                                (b.watch && b.watch.length) ? (React.createElement("div", { style: { display: 'flex', flexDirection: 'column', gap: 7 } }, b.watch.map((w, i) => (React.createElement("div", { key: i, style: { display: 'flex', gap: 9, alignItems: 'flex-start' } },
                                    React.createElement("span", { style: { width: 5, height: 5, borderRadius: 999, background: '#c4a93a', marginTop: 7, flexShrink: 0 } }),
                                    React.createElement("span", { style: { font: '600 12px/1.65 Pretendard', color: '#3d3e42' } }, w)))))) : (React.createElement("div", { style: { font: '500 12px/1.6 Pretendard', color: '#9a9ca0' } }, "\uAE30\uC900\uC120\uC744 \uB118\uB294 \uD2B9\uC774 \uC2E0\uD638\uB294 \uC5C6\uC5C8\uC74C"))),
                            React.createElement("div", { style: col2 },
                                React.createElement(BriefSection, { title: "\uAD6D\uB0B4\uC99D\uC2DC", rows: b.kr, live: lv, onPick: openTrend, note: "\uB204\uB974\uBA74 5\uB144 \uCD94\uC774" }),
                                React.createElement(BriefSection, { title: "\uD574\uC678\uC99D\uC2DC", rows: b.global, live: lv, onPick: openTrend }),
                                React.createElement(BriefSection, { title: "\uD658\uC728", rows: b.fx, digits: 4, live: lv, onPick: openTrend }),
                                React.createElement(BriefSection, { title: "\uC6D0\uC790\uC7AC", rows: b.commodity, live: lv, onPick: openTrend, note: "\uAE08\u00B7\uC740\u00B7\uC6D0\uC720 (USD)" }),
                                React.createElement(BriefSection, { title: "\uD06C\uB9BD\uD1A0", rows: b.crypto, live: lv, onPick: openTrend, note: "USD \uAE30\uC900 \u00B7 \uC2E4\uC2DC\uAC04" }),
                                React.createElement("div", null,
                                    React.createElement("div", { style: { marginTop: 20 } },
                                        React.createElement("div", { style: { display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 8, flexWrap: 'wrap' } },
                                        ...
                                        (b.tenorRates && b.tenorRates.length) ? (React.createElement("div", { style: { display: 'flex', flexDirection: 'column', gap: 10 } }, Object.entries((b.tenorRates || []).reduce((m, r) => { (m[r.group] = m[r.group] || []).push(r); return m; }, {})).map(([g, rows]) => (React.createElement("div", { key: g, style: { border: '1px solid #ece9e2', borderRadius: 13, overflow: 'hidden' } },
                                            React.createElement("div", { style: { display: 'flex', alignItems: 'baseline', gap: 7, font: '700 11px Pretendard', color: '#56585c', background: '#f8f7f3', padding: '8px 13px' } },
                                                React.createElement("span", null, g),
                                                React.createElement("span", { style: { marginLeft: 'auto', font: '500 9px Pretendard', color: '#b6b8bc' } }, rows[0].src)),
                                            rows.map((r, i) => (React.createElement("div", { key: r.tenor + i, style: { display: 'flex', alignItems: 'center', gap: 10, padding: '9px 13px', borderTop: '1px solid #f3f1ea' } },
                                                ...
                                                React.createElement("span", { style: { font: '800 13.5px Pretendard' } },
                                                    r.rate.toFixed(3),
                                                    "%"),
                                                React.createElement("span", { style: { font: '500 9.5px Pretendard', color: '#b6b8bc', minWidth: 72, textAlign: 'right' } }, r.asOf))))))))) : (React.createElement("div", { style: { font: '500 11px/1.6 Pretendard', color: '#b6b8bc', border: '1px dashed #e3e0d8', borderRadius: 13, padding: '14px' } }, "\uAE08\uB9AC \uB370\uC774\uD130\uB97C \uBC1B\uC9C0 \uBABB\uD588\uC74C"))),
                                    React.createElement("div", { style: { marginTop: 18 } },
                                        React.createElement("div", { style: { font: '700 11px Pretendard', color: '#a6a8ac', letterSpacing: '.06em', marginBottom: 8 } }, "\uD55C\uAD6D \u00B7 \uBBF8\uAD6D \uAE30\uC900\uAE08\uB9AC"),
                                        React.createElement("div", { style: { border: '1px solid #ece9e2', borderRadius: 13, overflow: 'hidden' } },
                                        ...
                                        b.ust && b.ust.length > 0 && b.ust[0].last != null && (React.createElement("div", { onClick: () => openTrend(b.ust[0]), style: { display: 'flex', alignItems: 'center', gap: 8, marginTop: 8, cursor: 'pointer', font: '500 11px Pretendard', color: '#9a9ca0' } },
                            ...
                            React.createElement("div", { style: { marginTop: 24 } },
                                React.createElement("div", { style: { font: '700 11px Pretendard', color: '#a6a8ac', letterSpacing: '.06em', marginBottom: 8 } }, "\uC8FC\uC694\uC774\uC288 \uB274\uC2A4"),
                                (b.issues && b.issues.length) ? (React.createElement("div", { style: { border: '1px solid #ece9e2', borderRadius: 13, overflow: 'hidden', ...(isDesktop ? { display: 'grid', gridTemplateColumns: '1fr 1fr' } : {}) } }, b.issues.map((it, i) => (React.createElement("a", { key: i, href: it.url, target: "_blank", rel: "noopener noreferrer", style: { display: 'block', textDecoration: 'none', color: 'inherit', padding: '12px 13px', borderTop: '1px solid #f3f1ea' } },
                            ...
                            (b.errors && b.errors.length > 0) && (React.createElement("div", { style: { font: '500 10.5px/1.6 Pretendard', color: '#b6b8bc', marginTop: 16, background: '#f8f7f3', borderRadius: 9, padding: '10px 12px' } },
                                ...
                                b.errors.length,
                                "\uAC74 \u2014 \uAC12\uC744 \uCD94\uC815\uD558\uC9C0 \uC54A\uACE0 \uBE44\uC6CC \uB480\uC74C")),
                            React.createElement("div", { style: { marginTop: 14, border: '1px solid #ece9e2', borderRadius: 12, padding: '11px 13px' } },
                                React.createElement("div", { style: { font: '700 10px Pretendard', color: '#a6a8ac', letterSpacing: '.05em', marginBottom: 6 } }, "\uCD9C\uCC98"),
                                React.createElement("div", { style: { font: '500 10.5px/1.8 Pretendard', color: '#7a7c80' } },
                                    "\uC9C0\uC218\u00B7\uD658\uC728\u00B7\uC6D0\uC790\uC7AC\u00B7\uCD94\uC774 \uADF8\uB798\uD504 ",
                                    React.createElement("b", null, "Yahoo Finance"),
                                    "(\uD3F4\uBC31 Stooq) \u00B7 \uD06C\uB9BD\uD1A0 ",
                                    React.createElement("b", null, "Binance"),
                                    "(\uD3F4\uBC31 Coinbase) \u00B7 SOFR/EFFR\u00B7FOMC \uBAA9\uD45C\uBC94\uC704 ",
                                    React.createElement("b", null, "New York Fed"),
                                    " \u00B7 SONIA\u00B7\uC601\uAD6D \uC815\uCC45\uAE08\uB9AC ",
                                    React.createElement("b", null, "Bank of England"),
                                    " \u00B7 \u20ACSTR ",
                                    React.createElement("b", null, "ECB"),
                                    " \u00B7 EURIBOR ",
                                    React.createElement("b", null, "\uC77C\uBCC4 \uACF5\uD45C(euribor-rates.eu)"),
                                    " \u00B7 TONA ",
                                    React.createElement("b", null, "\uC77C\uBCF8\uC740\uD589"),
                                    " \u00B7 \uD55C\uAD6D \uAE30\uC900\uAE08\uB9AC ",
                                    React.createElement("b", null, "\uD55C\uAD6D\uC740\uD589"),
                                    " \u00B7 \uB274\uC2A4 ",
                                    React.createElement("b", null, "\uAD6C\uAE00 \uB274\uC2A4"),
                                    " \u00B7 \uD658\uD5E4\uC9C0\u00B7\uC2A4\uC651\uD3EC\uC778\uD2B8\uB294 \uC704 \uAE08\uB9AC\uB85C ",
                                    React.createElement("b", null, "\uAE08\uB9AC\uD3C9\uD615 \uACC4\uC0B0"))))))),
